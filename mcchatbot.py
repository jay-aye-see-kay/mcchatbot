...
class GameModeEvent(LogEvent):
    ...
    game_mode: str = ""
    def to_s(self):
        return  f"at \"{self.time}\" {self.username} set their game mode to {self.game_mode}\n"

def parse_event(line: str) -> LogEvent | None:
    match = re.search(r'\[(?P<time>\d{2}:\d{2}:\d{2}) INFO\]: (?:<(?P<username>.+?)> )?(?P<text>.*)', line)
    line_content = re.sub(r'^\[.*\]: ', '', line)
    game_mode_match = re.search(r'\[.*\]: \[(?P<username>.*): Set own game mode to (?P<game_mode>.*)\]', line)
    if not match:
        return
...
system_message = "You are the chacter Wheatley from Portal 2 chatting with people playing minecraft. \
Your responses must be formatted in plain text only. \
Previous messages include a timestamp and username only for your context, you must not include these in your responses. \
After someone leaves the game they cannot see what you say, but remaining players can. \
...
def respond_to_event(event: LogEvent):
    ...
    first_idx = 0 if len(all_messages) < CONTEXT_MSGS else len(all_messages) - CONTEXT_MSGS
    ...
    process_cmd = ["docker-compose", "exec", "business", "rcon-cli", "tellraw", "@a", f"\"<Wheatley> {response}\""]
    subprocess.run(process_cmd)

def listen_to_events():
    process = subprocess.Popen(['docker', 'logs', '-f', 'mc_business'], stdout=subprocess.PIPE, universal_newlines=True)
    try:
